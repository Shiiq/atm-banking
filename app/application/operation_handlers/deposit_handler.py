...
class Deposit(BaseHandler):
    ...
    async def execute(
        ...
        async with self._uow:
            ...
            customer, current_account = await self._customer_service.by_fullname(
        ...
        return ShortOperationInfo(created_at=operation.created_at,
        ...
        #         customer, current_account = await self._customer_service.by_fullname(search_data=customer_search_data)
        #         _logger.info(
    ...
    async def _update_bank_account(
            self,
            current_account: BankAccountRead,
            operation_amount: int
    ) -> BankAccountRead:

        balance = current_account.balance + operation_amount
        updates_for_account = BankAccountUpdate(
            id=current_account.id,
            balance=balance
        )
        updated_account = await self._account_service.update(
            updates_for_account=updates_for_account
        )
        return updated_account
